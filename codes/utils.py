...
def model_result_store(y, y_pred, result_store, kfold):
    ...
    rocauc              = np.round(metrics.roc_auc_score(y_true=y, y_score=y_pred), 4)
    # roc                 = np.round(metrics.roc_curve(y_true=y, y_score=y_pred), 4)

    
    with open(result_store, 'a', newline='') as csvfile:
        fieldnames = ['fold', 'Accuracy','F1', 'Sensitivity', 'Specificity', 'PPV', 'NPV', 'rocauc']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writerow({'fold': kfold, 'Accuracy': Accuracy, 'F1': F1, 'Sensitivity': Sensitivity, 'Specificity': Specificity, 'PPV' : PPV, 'NPV' : NPV, 'rocauc' : rocauc})
                
    return 

def model_result_store_regression(y, y_pred, result_store, kfold):

    mse    = np.round(metrics.mean_squared_error(y, y_pred), 4)
    rmse   = np.round(metrics.mean_squared_error(y, y_pred, squared=False), 4)
    mae    = np.round(metrics.mean_absolute_error(y, y_pred), 4)
    r2_score = np.round(metrics.r2_score(y, y_pred), 4)

    with open(result_store, 'a', newline='') as csvfile:
        fieldnames = ['fold', 'mse', 'mae', 'rmse', 'r2_score']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writerow({'fold': kfold, 'mse': mse, 'mae': mae, 'rmse': rmse, 'r2_score': r2_score})
...
def filter_ecg(ecg, ecg_sampling_freq):
    b, a = scipy_signal.iirdesign(wp = 0.8, ws = 0.4, gpass = 1, gstop = 60, ftype = 'cheby2', fs = ecg_sampling_freq)
