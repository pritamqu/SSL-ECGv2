...
def model_classification(x_tr_feature, 
                        y_tr, 
                        x_te_feature, 
                        y_te, 
                        identifier, 
    ...
    pred_dir= os.path.join(result, 'prediction')
    result  = os.path.join(result, 'ER_result')
    ...
    model.fit(x_tr_feature, y_tr, epochs=epoch_super, batch_size=batch_super, callbacks=[tb], verbose=2, validation_data = (x_te_feature, y_te), shuffle = True) 
    y_tr_pred = model.predict(x_tr_feature, batch_size=batch_super)
    y_te_pred = model.predict(x_te_feature, batch_size=batch_super)
    
    np.save(os.path.join(pred_dir, 'te_pred_' + identifier + '_fold_' + str(kfold) + '.npy'), y_te_pred)
    np.save(os.path.join(pred_dir, 'te_real_' + identifier + '_fold_' + str(kfold) + '.npy'), y_te)
...
def model_regression(x_tr_feature, 
                    y_tr, 
                    x_te_feature, 
                    y_te, 
                    identifier, 
    ...
    model = keras.models.Sequential()
    model.add(keras.layers.Dense(hidden_nodes, input_dim=input_dimension, activation='relu', kernel_regularizer = keras.regularizers.l2(L2)))
    model.add(keras.layers.Dense(hidden_nodes//1, activation='relu', kernel_regularizer = keras.regularizers.l2(L2)))
    model.add(keras.layers.Dense(hidden_nodes//1, activation='relu', kernel_regularizer = keras.regularizers.l2(L2)))
    model.add(keras.layers.Dense(hidden_nodes//1, activation='relu', kernel_regularizer = keras.regularizers.l2(L2)))


    model.add(keras.layers.Dense(output_dimension, activation=None))
    op = keras.optimizers.Adam(lr=lr_super)
    ...
    np.save(os.path.join(pred_dir, 'te_pred' + identifier + '_fold_' + str(kfold) + '.npy'), y_te_pred)
    np.save(os.path.join(pred_dir, 'te_real_' + identifier + '_fold_' + str(kfold) + '.npy'), y_te)

    utils.model_result_store_regression(y_tr, y_tr_pred, os.path.join(result, str("tr_" + identifier + ".csv")), kfold)
    utils.model_result_store_regression(y_te, y_te_pred, os.path.join(result, str("te_" + identifier + ".csv")), kfold)
